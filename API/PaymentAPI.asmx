...
public class PaymentAPI : System.Web.Services.WebService {
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public EWin.PaymentBankCard.APIResult CompleteReceiptFile(string WebSID, string GUID, string UploadId) {
        RedisCache.SessionContext.SIDInfo SI;
        EWin.PaymentBankCard.APIResult R = new EWin.PaymentBankCard.APIResult();


        SI = RedisCache.SessionContext.GetSIDInfo(WebSID);

        if (SI != null && !string.IsNullOrEmpty(SI.EWinSID)) {
            EWin.PaymentBankCard.PaymentBankCardAPI paymentBankCardAPI = new EWin.PaymentBankCard.PaymentBankCardAPI();
            //PaymentMethod 0=上分/1=下分
            R= paymentBankCardAPI.CompleteReceiptFile(SI.EWinCT,GUID,UploadId);

        } else {
            R.ResultState = EWin.PaymentBankCard.enumResultState.ERR;
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public EWin.PaymentBankCard.UploadInfoResult CreateReceiptFIleUpload(string WebSID, string GUID, string PaymentSerial, string ExtName) {
        RedisCache.SessionContext.SIDInfo SI;
        EWin.PaymentBankCard.UploadInfoResult R = new EWin.PaymentBankCard.UploadInfoResult();
        ...
        if (SI != null && !string.IsNullOrEmpty(SI.EWinSID)) {
            EWin.PaymentBankCard.PaymentBankCardAPI paymentBankCardAPI = new EWin.PaymentBankCard.PaymentBankCardAPI();
            //PaymentMethod 0=上分/1=下分
            R= paymentBankCardAPI.CreateReceiptFIleUpload(SI.EWinCT,GUID,PaymentSerial,ExtName);
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public EWin.PaymentBankCard.APIResult UploadReceiptFIle(string WebSID, string GUID, string UploadId, int ChunkIndex, string ContentB64) {
        RedisCache.SessionContext.SIDInfo SI;
        EWin.PaymentBankCard.APIResult R = new EWin.PaymentBankCard.APIResult();
        ...
        if (SI != null && !string.IsNullOrEmpty(SI.EWinSID)) {
            EWin.PaymentBankCard.PaymentBankCardAPI paymentBankCardAPI = new EWin.PaymentBankCard.PaymentBankCardAPI();
            //PaymentMethod 0=上分/1=下分
            R= paymentBankCardAPI.UploadReceiptFIle(SI.EWinCT,GUID,UploadId,ChunkIndex,ContentB64);
                            ...
                            if (Amount >= MinLimit) {
                                if (Amount <= MaxLimit || MaxLimit == 0) {
                                    if ((int)PaymentMethodDT.Rows[0]["EWinPaymentType"] == 1) {
                                        ...
                                        EWin.PaymentBankCard.PaymentBankCardAPI paymentBankCardAPI = new EWin.PaymentBankCard.PaymentBankCardAPI();
                                        //PaymentMethod 0=上分/1=下分
                                        EWin.PaymentBankCard.BankCardDepositResult bankCardDepositRet= paymentBankCardAPI.BankCardDepostit(SI.EWinCT,GUID,EWinWeb.MainCurrencyType,0,Amount,"");

                                        if (bankCardDepositRet.ResultState == EWin.PaymentBankCard.enumResultState.OK)
                                        {
